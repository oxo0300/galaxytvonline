...
class AdvSearchResults extends AdvSearchUtil{
    ...
    public function doSearch($asContext) {
		...
		if ($this->mainClass == 'modResource') {
			...
			$results = ($engine == 'all') ? $this->doAllSearch() : $this->doEngineSearch($engine);
		}
		else {
			// search in a different main package and possibly joined packages
			$results = $this->customSearch();
		}
	 ...
	 */
     public function doEngineSearch($engine) {
		...
		if ($engine == 'mysql' || $lstIds){
			...
			// hideMenu
			if ($this->config['hideMenu'] == 0) $c->andCondition(array('hidemenu' => '0'));
			elseif ($this->config['hideMenu'] == 1) $c->andCondition(array('hidemenu' => '1'));

			// hideContainers
			if ($this->config['hideContainers']) $c->andCondition(array('isfolder' => '0'));

			// restrict search to $lstIds
			if (!empty($lstIds)) $c->andCondition(array("modResource.id IN (" . $lstIds . ")"));

			// multiple context support
			if (!empty($this->config['contexts'])) {
				$contextArray = explode(',',$this->config['contexts']);
				$contexts = array();
				foreach($contextArray as $ctx) $contexts[] = $this->modx->quote($ctx);
			...
			if ($engine == 'mysql') {
				if (!empty($this->searchQuery)) {
					$condition = $this->getMysqlQuery($this->searchQuery);
					$c->andCondition($condition);
				}
			}

			//============================= add query conditions
			if (!empty($this->queryHook['andConditions'])) $c->andCondition($this->queryHook['andConditions']);
			...
			if (empty($this->queryHook['stmt'])) {
				// debug mysql query
				if ($this->dbg) $this->modx->log(modX::LOG_LEVEL_DEBUG, 'SearchString: '.$this->searchString,'','doEngineSearch');
				if ($this->dbg) $this->modx->log(modX::LOG_LEVEL_DEBUG, 'Select before pagination: '.$this->niceQuery($c),'','doEngineSearch');
	 ...
	 */
    private function initSearch($engine, & $lstIds, & $hits) {
        if ($engine == 'zend') {
		...
		if ($this->dbg) $this->modx->log(modX::LOG_LEVEL_DEBUG, 'lstIds: '.$lstIds,'','initSearch');
	 ...
	 */
	function getZendQuery($query, $sign = true) {
		if ( $query instanceOf Zend_Search_Lucene_Search_Query_Boolean ) {
			...
			for($i=0;$i<$nbs;$i++) {
				...
				if (is_null($signs[$i])) $orCondition[] = $condition;
				elseif ($signs[$i]) $andCondition[] = $condition;
				elseif (!$signs[$i]) $notCondition[] = $condition;
			}
			$conditions = array();
			$zendCondition = '';
			$nband = count($andCondition);
			if ($nband) $conditions[] = ($nband > 1) ? '('.implode(' AND ',$andCondition).')'  : $andCondition[0];
			$nbor = count($orCondition);
			if ($nbor) $conditions[] = ($nbor > 1) ? '('.implode(' OR ',$orCondition).')' : $orCondition[0];
			$nbnot = count($notCondition);
			if ($nbnot) $conditions[] = ($nbnot > 1) ? 'NOT('.implode(' AND ',$notCondition).')' : 'NOT '.$notCondition[0];

			$nbc = count($conditions);
			if ($nbc) $zendCondition = ($nbc >1) ? '('.implode(' AND ', $conditions).')' : $conditions[0];
		...
		elseif( $query instanceOf Zend_Search_Lucene_Search_Query_Preprocessing_Term ) {
			...
			if ($sign || is_null($sign)) $term = '*'.$term.'*'; // NOT excluded
			return $term;
		}
	}
	 ...
	 */
    private function addQuerySearchConditions($engine, xPDOQuery $c, $searchQuery) {
        if ($engine == 'mysql') {
            if (!empty($searchQuery)) {
                $condition = $this->getMysqlQuery($searchQuery);
                $c->andCondition($condition);
            }
        }
        return $c;
	 ...
	 * @return string Returns a condition
	 */
    private function getMysqlQuery($query = null, $sign = true) {
		if ( $query instanceOf Zend_Search_Lucene_Search_Query_Boolean ) {
			$orCondition = array();
			$andCondition = array();
			$notCondition = array();
			...
			$nbs = count($subqueries);
			for($i=0;$i<$nbs;$i++) {
				$condition = $this->getMysqlQuery($subqueries[$i], $signs[$i]);
				if (is_null($signs[$i])) $orCondition[] = $condition;
				elseif ($signs[$i]) $andCondition[] = $condition;
				elseif (!$signs[$i]) $notCondition[] = $condition;
			}
			...
			if ($nbor)  $conditions[] = ($nbor > 1) ? '(('.implode(') OR (',$orCondition).'))' : $orCondition[0];
			$nbnot = count($notCondition);
			if ($nbnot)  $conditions[] = ($nbnot > 1) ? '(('.implode(') AND (',$notCondition).'))' : $notCondition[0];

			$nbc = count($conditions);
			if ($nbc) $mysqlCondition = ($nbc >1) ? '('.implode(' AND ', $conditions).')' : $conditions[0];
            ...
            $condition = $this->setCondition($phrase, $sign);
			return $condition;
		}
		else if ( $query instanceOf Zend_Search_Lucene_Search_Query_Preprocessing_Term ) {
			$term = $query->__toString();
            $condition = $this->setCondition($term, $sign);
			return $condition;
		}
	}

    private function setCondition($term, $sign) {
		$isLikeCondition = (($sign) || is_null($sign));
		if ($isLikeCondition) $like = 'LIKE';
		else $like = 'NOT LIKE';
		...
		$term = "%{$term}%";
		$orand = array();
		foreach($this->mainWhereFields as $field) $orand[] = "`{$this->mainClass}`.`{$field}` {$like} '{$term}'";
		if (!empty($this->tvWhereFields)) foreach($this->tvWhereFields as $field) $orand[] = "`{$field}_cv`.`value` {$like} '{$term}'";
		if (!empty($this->joinedWhereFields)) foreach($this->joinedWhereFields as $field) $orand[] = "{$field} {$like} '{$term}'";
		$condition = '';
        if (count($orand)) {
			if ($isLikeCondition) $condition = '((' . implode(') OR (',$orand) . '))';
			else $condition = '((' . implode(') AND (',$orand) . '))';
		}
		return $condition;
	}
	 ...
	 * @return array Returns an array of results
     */
    private function appendTVsFields($collection) {
		$results = array();
     ...
     */
    private function checkResultsParams() {

		if (!empty($this->queryHook['main'])){ // a new main package is declared in query hook
			$msg = '';
			if (empty($this->queryHook['main']['package'])) $msg = 'Main - Package name should be declared in queryHook';
			elseif (empty($this->queryHook['main']['packagePath'])) $msg = 'Main - Package path should be declared in queryHook';
			elseif (empty($this->queryHook['main']['class'])) $msg = 'Main - Class name should be defined in queryHook';
			if (!empty($msg)) {
				$this->modx->log(modX::LOG_LEVEL_ERROR, '[AdvSearch] '.$msg,'','checkResultsParams');
				return false;
			}
		...
		if ($this->mainClass == 'modResource') $requiredFields = array('id', 'template', 'context_key', 'createdon');
		else $requiredFields = array($this->primaryKey);
        ...
        else $this->config['fieldPotency'] = $lstFieldPotency;
		...
		if ((!empty($this->queryHook)) && (!empty($this->queryHook['perPage']))) $perPage = $this->queryHook['perPage'];
        ...
        else $this->config['sortby'] = $lstSortby;

        if ($this->dbg) $this->modx->log(modX::LOG_LEVEL_DEBUG,'[AdvSearch] Config parameters after checking: '.print_r($this->config, true),'','checkResultsParams');
    ...
    private function customSearch() {
		...
		if (!empty($this->config['ids'])) $c->andCondition(array("{$this->mainClass}.{$this->primaryKey} IN (".$this->config['ids'].")"));

		// restrict search with where condition
		if (!empty($main['where'])) {
			if (!is_array($main['where'])) $c->andCondition(array($main['where']));
			else $c->andCondition($main['where']);
		}
		...
		// debug mysql query
		if ($this->dbg) $this->modx->log(modX::LOG_LEVEL_DEBUG, 'SearchString: '.$this->searchString,'','customSearch');
		if ($this->dbg) $this->modx->log(modX::LOG_LEVEL_DEBUG, 'Select before pagination: '.$this->niceQuery($c),'','customSearch');

		// get number of results before pagination
		$this->resultsCount = $this->modx->getCount($main['class'], $c);
		if ($this->dbg) $this->modx->log(modX::LOG_LEVEL_DEBUG, 'Number of results before pagination: '.$this->resultsCount,'','customSearch');
		...
		if ($this->resultsCount > 0) {
			...
			// debug mysql query
			if ($this->dbg) $this->modx->log(modX::LOG_LEVEL_DEBUG, 'Final select: '.$this->niceQuery($c),'','customSearch');
		...
		if ($this->dbg) {
            $this->modx->log(modX::LOG_LEVEL_DEBUG, "lstIdsResults:" . $lstIdResults,'','customSearch');
		}
		return $this->results;
	}
     ...
     */
    private function addJoinedResources(xPDOQuery $c) {
