...
class VisionCart {
    ...
    public function route($config=array()) {
		$categories = $this->router['categories'];
		if (isset($this->router['product']['id']) && is_numeric($this->router['product']['id']) && $this->router['product']['id'] != 0) {
			if (count($this->router['categories']) > 1) {
				$this->category = $this->getCategory(end($this->router['categories']), array(
					'whereColumn' => 'alias',
					'queryAnd' => array(
						'parent' => $this->getCategory(prev($this->router['categories']), array(
							'whereColumn' => 'alias'
						))->get('id')
					)
				));
			...
			if ($this->product != null) {
				$this->product->set('url', $this->makeUrl(array(
					'productCategory' => (int) $this->product->getOne('ProductCategory', array(
    ...
    public function getThumbnailSettings($shopId, $category=false) {
    	...
    	$hash = sha1(json_encode($shopId.$category));
    	 
    	if (!isset($settings[$hash])) {
	    	if ($category) {
     ...
     *
     * @access public
     * @param string $prefix Prefix of the snippet
     ...
     * @return bool Success
     */
    public function fireEvent($prefix, $event, $snippetParameters = array(), $config = array()) {
    	static $events, $eventSnippets;
    	
		$config['offset'] = $this->modx->getOption('offset', $config, 0);
		$config['query'] = $this->modx->getOption('query', $config, array());
		
		// Query defaults
		$conditions =& $config['query'];
		$conditions['sort'] = $this->modx->getOption('sort', $conditions['sort'], array('column' => 'name', 'direction' => 'ASC'));
		$conditions['sort']['column'] = $this->modx->getOption('column', $conditions['sort'], 'name');
		$conditions['sort']['direction'] = $this->modx->getOption('direction', $conditions['sort'], 'ASC');
		
		$conditionHash = sha1($event.json_encode($conditions));
		$snippetCollectionHash = sha1($prefix.json_encode($conditions));
		
		// First fetch the snippets for the prefix
		if(!isset($eventSnippets[$snippetCollectionHash])) {
			$query = $this->modx->newQuery('modSnippet', array(
				'name:LIKE' => $prefix.'%'
			));
			
			$query->sortby($conditions['sort']['column'], $conditions['sort']['direction']);
			
			$snippets = $this->modx->getCollection('modSnippet', $query);
			$eventSnippets[$snippetCollectionHash] = $snippets;
		}
		
		$internal['count'] = 0;
		
		if(sizeof($eventSnippets[$snippetCollectionHash]) != 0) {
			foreach($eventSnippets[$snippetCollectionHash] as $key => $snippet) {
				if($config['offset'] != 0 && $internal['count'] < $config['offset']) {
					continue;
				}
				
				// If the snippet is not active, skip it second time around ;)
				if($snippet->get('active') == false) {
					continue;
				}
				
				$response = $this->modx->fromJSON($this->modx->runSnippet($snippet->get('name'), array(
					'vcAction' => 'getParams'
				)));
								
				// Skip if the snippet is set as inactive in the getParams (snippets don't have disable functionality like plugins)
				if(isset($response['active']) && $response['active'] == false) {
					// Set the snippet inactive so it will only get run once
					$eventSnippets[$snippetCollectionHash][$key]->set('active', false);
					continue;
				}
				...
				if($response['event'] == $event || $event == '') {
					$this->modx->runSnippet($snippet->get('name'), $snippetParameters);
				}
    ...
    public function makeUrl($config=array()) {
        ...
        if (isset($config['productCategory'])) {
        	$folder = 'product/'.$config['productCategory'];
        } else {
        	$folder = 'category/'.$config['categoryId'];
        }
        
        $cacheHash = sha1(json_encode($config));
        $cache = $this->getCache($cacheHash, 'makeUrl/'.$folder);
        ...
        $this->setCache($cacheHash, $url, 0, 'makeUrl/'.$folder);
    ...
    public function wayFinder($scriptProperties=array(), $config=array()) {
		...
		$config['excludeProducts'] = $this->modx->getOption('excludeProducts', $config, '');
		...
		// Added 2011-08-10 (Mark) Exclude categories (and those IDs) with the query to gain loads of speed
		if (!empty($config['excludeCategories'])) {
			$query->andCondition(array(
				'id:NOT IN' => explode(',', $config['excludeCategories'])
			));
		}
    ...
    public function calculateProductPrice($product, $asArray=false, $cached=true) {
    	static $productPrices, $taxCategories;
    	...
    	// Set a default cachingkey
    	$product->set('cachekey', 'calcprodprice');
    	$product->set('cacheable', true);
    	...
    	$cacheHash = sha1('product_'.$product->get('id').'_'.intval($asArray).'_'.$product->get('cachekey'));
    	$cache = $this->getCache($cacheHash, 'product/'.$product->get('id'));

    	if ($cache != null && $product->get('cacheable') == true) {
    		$product->set('price', $cache);
    	} else {
	    	// Get all links to categories for this product
	    	$productCategories = $this->modx->getCollection('vcProductCategory', array(
	    		'productid' => $product->get('id')
	    	));
	    	...
	    	if ($product->get('cacheable')) {
	    		$this->setCache($cacheHash, $cheapest, 0, 'product/'.$product->get('id'));
	    	}
    	}
    	...
    	// Get the tax category
    	if(!isset($taxCategories[$product->get('taxcategory')])) {
    		$taxCategories[$product->get('taxcategory')] = $product->getOne('TaxCategory');
    	}
    	$taxCategory = $taxCategories[$product->get('taxcategory')];
     ...
     *
     * @access public
     * @param string $cacheHash The unique cache hash for the element
     * @param array $cacheKey Defaults to 'visioncart'. Enter a path here (eg. product/$id)
     * @return mixed The cache contents or NULL if no cache exists
     */
    public function getCache($cacheHash, $cacheKey = 'visioncart') {
    	return $this->modx->cacheManager->get('visioncart.'.$cacheHash, array(
			xPDO::OPT_CACHE_KEY => 'visioncart/'.$cacheKey
		));
    }
     ...
     * @access public
     * @param string $cacheHash The unique cache hash for the element
     * @param array $cacheValue The contents for the cache file
     * @param array $timeToLive Time to live in seconds for the cache file. Defaults to 0
     * @param array $cacheKey Defaults to 'visioncart'. Enter a path here (eg. product/$id)
     * @return void
     */
    public function setCache($cacheHash, $cacheValue, $timeToLive = 0, $cacheKey = 'visioncart') {
    	return $this->modx->cacheManager->set('visioncart.'.$cacheHash, $cacheValue, $timeToLive, array(
			xPDO::OPT_CACHE_KEY => 'visioncart/'.$cacheKey
		));
    }
